...
При написании пользовательских утверждений лучше всего следовать принципам реализации собственных утверждений PHPUnit. Как вы можете видеть в [Пример 12.1](https://phpunit.readthedocs.io/ru/latest/extending-phpunit.html#extending-phpunit-examples-assert-php), метод`assertTrue()` — это просто обёртка над методами `isTrue()` и `assertThat()`: `isTrue()`создаёт объект сопоставления, который передаётся `assertThat()` для проведения вычисления.
...
Интерфейс `PHPUnit\Framework\Test` — небольшой и простой для реализации. Вы можете написать реализацию `PHPUnit\Framework\Test`, которая проще, чем`PHPUnit\Framework\TestCase`, и которая, например, запускает _тесты, управляемые данными_.
...
[Пример 12.6](https://phpunit.readthedocs.io/ru/latest/extending-phpunit.html#extending-phpunit-examples-testrunnerextension) показывает пример расширения, реализующего `BeforeFirstTestHook` и `AfterLastTestHook`:
...
[https://phpunit.readthedocs.io/ru/latest/extending-phpunit.html](https://phpunit.readthedocs.io/ru/latest/extending-phpunit.html)
